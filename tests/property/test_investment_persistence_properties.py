...
class TestInvestmentPersistenceProperties:
    ...
    def test_investment_persistence_round_trip(self, investments: List[Investment]):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for original, loaded in zip(original_sorted, loaded_sorted):
                assert loaded.date == original.date, f"Date mismatch: {loaded.date} != {original.date}"
                assert loaded.ticker == original.ticker, f"Ticker mismatch: {loaded.ticker} != {original.ticker}"
                assert abs(loaded.price - original.price) < 0.01, f"Price mismatch: {loaded.price} != {original.price}"
                assert abs(loaded.amount - original.amount) < 0.01, f"Amount mismatch: {loaded.amount} != {original.amount}"
                assert abs(loaded.shares - original.shares) < 0.0001, f"Shares mismatch: {loaded.shares} != {original.shares}"
    ...
    def test_constraint_checking_with_persisted_data(self, investments: List[Investment], check_date: date):
        """
        Feature: buy-the-dip-strategy, Property 8: Investment Persistence Round-Trip
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert result1 == expected_result, f"Constraint check result should match expected: {result1} != {expected_result}"
