...
class TestPriceCachingProperties:
    ...
    def test_cache_merging_correctness(
        self, ticker: str, num_days_first: int, num_days_second: int, base_price: float
    ):
        """
        ...
        with self.temp_cache_dir() as temp_cache_dir:
            ...
            start_date_second = base_date + timedelta(
                days=max(0, num_days_first - 3)
            )  # Small overlap
            ...
            if not combined_result.empty:
                ...
                first_dates = (
                    set(first_result["Date"].tolist()) if not first_result.empty else set()
                )
                second_dates = (
                    set(second_result["Date"].tolist()) if not second_result.empty else set()
                )
    ...
    def test_persistent_cache_round_trip(self, ticker: str, num_days: int, base_price: float):
        """
        Feature: buy-the-dip-strategy, Property 2: Price Data Caching Correctness
    ...
    def test_cache_expiration_logic(
        self, ticker: str, cache_days_valid: int, cache_days_invalid: int
    ):
        """
        Feature: buy-the-dip-strategy, Property 2: Price Data Caching Correctness
