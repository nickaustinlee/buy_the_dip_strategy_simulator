...
class TestInvestmentConstraintProperties:
    ...
    def test_investment_constraint_enforcement(
        self, investment_dates: List[date], check_date: date
    ):
        """
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for investment_date in unique_dates:
                ...
                should_be_allowed = not investment_tracker.has_recent_investment(
                    investment_date, days=28
                )

                if should_be_allowed:
                    # Simulate successful investment
                    investment = Investment(
                        date=investment_date, ticker="SPY", price=150.0, amount=2000.0, shares=13.33
                    )
            ...
            expected_has_recent = any(
                cutoff_date < inv_date < check_date for inv_date in successful_investments
            )

            assert (
                has_recent == expected_has_recent
    ...
    def test_constraint_prevents_investments_within_28_days(
        self, base_date: date, days_between: int
    ):
        """
        Feature: buy-the-dip-strategy, Property 5: Investment Constraint Enforcement
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            first_investment = Investment(
                date=base_date, ticker="SPY", price=150.0, amount=2000.0, shares=13.33
            )
    ...
    def test_constraint_allows_investments_after_28_days(self, base_date: date, days_between: int):
        """
        Feature: buy-the-dip-strategy, Property 5: Investment Constraint Enforcement
    ...
    def test_constraint_uses_calendar_days_not_trading_days(self, investment_dates: List[date]):
        """
        Feature: buy-the-dip-strategy, Property 5: Investment Constraint Enforcement
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            cutoff_date = check_date - timedelta(days=28)
            expected_has_recent = any(
                cutoff_date < inv_date < check_date for inv_date in unique_dates[:-1]
            )
    ...
    @given(base_date=st.dates(min_value=date(2023, 2, 1), max_value=date(2023, 11, 1)))
    def test_constraint_boundary_conditions(self, base_date: date):
        """
        Feature: buy-the-dip-strategy, Property 5: Investment Constraint Enforcement
        ...
        test_cases = [
            ...
            (
                ...
                False,
            ),  # 28 days later - should be allowed (same weekday)
    ...
    def test_constraint_maintained_as_invariant(self, investment_sequence: List[tuple]):
        """
        Feature: buy-the-dip-strategy, Property 5: Investment Constraint Enforcement
    ...
    @given(check_date=st.dates(min_value=date(2023, 1, 29), max_value=date(2023, 12, 31)))
    def test_constraint_with_no_investment_history(self, check_date: date):
        """
        Feature: buy-the-dip-strategy, Property 5: Investment Constraint Enforcement
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert not has_recent, f"Should not have recent investment with empty history"
            ...
            assert should_invest, f"Should invest with no investment history and trigger met"
    ...
    def test_constraint_consistency_across_multiple_checks(
        self, investment_date: date, check_dates: List[date]
    ):
        """
        Feature: buy-the-dip-strategy, Property 5: Investment Constraint Enforcement
