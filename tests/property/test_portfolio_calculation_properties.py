...
class TestPortfolioCalculationProperties:
    ...
    def test_portfolio_calculation_correctness(self, investments: List[Investment], current_price: float):
        """
        ...
        if not investments:
            ...
            assert metrics.percentage_return == 0.0, "Empty portfolio should have zero percentage return"
        else:
    ...
    def test_portfolio_calculation_with_consistent_shares(self, investment_data: List[tuple], current_price: float):
        """
        Feature: buy-the-dip-strategy, Property 7: Portfolio Calculation Correctness
        ...
        assert abs(metrics.current_value - calculated_current_value) < 0.01, \
        ...
        assert abs(metrics.total_return - calculated_total_return) < 0.01, \
        ...
        if metrics.total_invested > 0:
            ...
            assert abs(metrics.percentage_return - calculated_percentage_return) < 0.01, \
    ...
    def test_portfolio_value_scales_with_price(self, base_investment: Investment, price_multiplier: float):
        """
        Feature: buy-the-dip-strategy, Property 7: Portfolio Calculation Correctness
        ...
        assert abs(scaled_metrics.total_invested - base_metrics.total_invested) < 0.01, \
            "Total invested should not change with price changes"
        
        # Total shares should remain constant
        assert abs(scaled_metrics.total_shares - base_metrics.total_shares) < 0.0001, \
        ...
        assert abs(scaled_metrics.current_value - expected_scaled_value) < 0.01, \
        ...
        assert abs(scaled_metrics.total_return - expected_scaled_return) < 0.01, \
    ...
    def test_portfolio_metrics_are_additive(self, investments: List[Investment], current_price: float):
        """
        Feature: buy-the-dip-strategy, Property 7: Portfolio Calculation Correctness
        ...
        # Verify additivity
        assert abs(full_metrics.total_invested - total_invested_sum) < 0.01, \
            "Total invested should be additive across investments"
        
        assert abs(full_metrics.total_shares - total_shares_sum) < 0.0001, \
            "Total shares should be additive across investments"
        
        assert abs(full_metrics.current_value - current_value_sum) < 0.01, \
        ...
        assert abs(full_metrics.total_return - expected_total_return) < 0.01, \
    ...
    def test_portfolio_break_even_point(self, investment: Investment):
        ...
        assert abs(metrics.percentage_return) < 0.01, \
        ...
        assert abs(metrics.current_value - metrics.total_invested) < 0.01, \
    ...
    def test_portfolio_average_cost_basis_calculation(self, investments: List[Investment]):
        ...
        if metrics.total_shares > 0:
            ...
            assert average_cost_basis >= min_price - 0.01, \
                f"Average cost basis {average_cost_basis} should be >= min price {min_price}"
            assert average_cost_basis <= max_price + 0.01, \
                f"Average cost basis {average_cost_basis} should be <= max price {max_price}"
            
            # Verify the calculation is mathematically correct
            expected_weighted_average = sum(inv.amount for inv in investments) / sum(inv.shares for inv in investments)
            assert abs(average_cost_basis - expected_weighted_average) < 0.01, \
