...
class TestTriggerCalculationProperties:
    ...
    def test_trigger_price_calculation_accuracy(
        ...
        assume(
            all(
                change == change and change != float("inf") and change != float("-inf")
        ...
        assert (
            abs(trigger_price - expected_trigger_price) < 0.01
        ...
        assert (
            trigger_price <= rolling_max + 0.01
        ...
        assert (
            abs(trigger_price / rolling_max - percentage_trigger) < 0.001
        ), f"Trigger price should equal rolling_max * percentage_trigger"

    @given(
        prices=st.lists(
            st.floats(min_value=50.0, max_value=500.0, exclude_min=True), min_size=30, max_size=100
        ),
    ...
    def test_trigger_price_uses_only_closing_prices(
        self, prices: List[float], window_days: int, percentage_trigger: float
    ):
        """
        Feature: buy-the-dip-strategy, Property 3: Trigger Price Calculation Accuracy
        ...
        assert (
            abs(trigger_price - manual_trigger_price) < 0.01
        ), f"Trigger price should be calculated using only closing prices"

    @given(
        base_prices=st.lists(
            st.floats(min_value=100.0, max_value=300.0, exclude_min=True), min_size=50, max_size=150
        ),
    ...
    def test_trigger_price_recalculated_fresh_each_day(
        self, base_prices: List[float], window_days: int, percentage_trigger: float
    ):
        """
        Feature: buy-the-dip-strategy, Property 3: Trigger Price Calculation Accuracy
    ...
    def test_trigger_price_handles_insufficient_data(
        ...
        assert trigger_price <= max(
            prices
        ), "Trigger price should not exceed maximum available price"
    ...
    def test_trigger_price_handles_empty_price_series(
        self, window_days: int, percentage_trigger: float
    ):
        """
        Feature: buy-the-dip-strategy, Property 3: Trigger Price Calculation Accuracy
        ...
        with pytest.raises(
            ValueError, match="Cannot calculate trigger price with empty price data"
        ):
    ...
    def test_trigger_price_with_constant_prices(
        self, constant_price: float, num_days: int, window_days: int, percentage_trigger: float
    ):
        """
        Feature: buy-the-dip-strategy, Property 3: Trigger Price Calculation Accuracy
