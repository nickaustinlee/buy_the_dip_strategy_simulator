...
class TestInvestmentDecisionProperties:
    ...
    def test_investment_decision_logic_correctness(
        ...
        assume(yesterday_price == yesterday_price and yesterday_price != float('inf') and yesterday_price != float('-inf'))
        assume(trigger_price == trigger_price and trigger_price != float('inf') and trigger_price != float('-inf'))
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            should_invest = strategy_system.should_invest(yesterday_price, trigger_price, evaluation_date)
            ...
            if trigger_met and not has_recent_investment:
                ...
                assert should_invest, \
                    "Should invest when trigger met and no recent investment"
    ...
    def test_investment_decision_independence_across_days(
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for eval_date in evaluation_dates:
                should_invest = strategy_system.should_invest(yesterday_price, trigger_price, eval_date)
            ...
            for i, (eval_date, should_invest) in enumerate(decisions):
                ...
                recent_investment_exists = any(
                    ...
                    for prev_date, prev_decision in decisions[:i] if prev_decision
                )
    ...
    def test_no_investment_when_trigger_not_met(
        ...
        assume(price_above_trigger == price_above_trigger and price_above_trigger != float('inf') and price_above_trigger != float('-inf'))
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            should_invest_no_recent = strategy_system.should_invest(yesterday_price, trigger_price, evaluation_date)
            assert not should_invest_no_recent, \
                f"Should not invest when yesterday_price ({yesterday_price}) > trigger_price ({trigger_price})"
            ...
            should_invest_with_recent = strategy_system.should_invest(yesterday_price, trigger_price, evaluation_date)
            assert not should_invest_with_recent, \
    ...
    def test_no_investment_when_recent_investment_exists(
        ...
        # Skip NaN and infinite values
        assume(base_price == base_price and base_price != float('inf') and base_price != float('-inf'))
        assume(price_below_trigger == price_below_trigger and price_below_trigger != float('inf') and price_below_trigger != float('-inf'))
