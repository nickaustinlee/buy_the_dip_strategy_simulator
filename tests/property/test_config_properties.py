...
class TestConfigurationProperties:
    ...
    def test_configuration_validation_rejects_invalid_ranges(
        self, rolling_window_days: int, percentage_trigger: float, monthly_dca_amount: float
    ):
        """
        Feature: buy-the-dip-strategy, Property 1: Configuration Loading and Validation Consistency
        ...
        assume(
            not (
                isinstance(percentage_trigger, float)
                and (
                    percentage_trigger != percentage_trigger
                    or percentage_trigger == float("inf")
                    or percentage_trigger == float("-inf")
                )
            )
        )
        assume(
            not (
                isinstance(monthly_dca_amount, float)
                and (
                    monthly_dca_amount != monthly_dca_amount
                    or monthly_dca_amount == float("inf")
                    or monthly_dca_amount == float("-inf")
                )
            )
        )
