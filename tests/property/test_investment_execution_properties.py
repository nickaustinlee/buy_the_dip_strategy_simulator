...
class TestInvestmentExecutionProperties:
    ...
    def test_investment_execution_and_recording_accuracy(
        self, evaluation_date: date, closing_price: float, monthly_dca_amount: float, ticker: str
    ):
        """
        ...
        assume(
            ...
            and closing_price != float("inf")
            and closing_price != float("-inf")
        )
        assume(
            monthly_dca_amount == monthly_dca_amount
            and monthly_dca_amount != float("inf")
            and monthly_dca_amount != float("-inf")
        )
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert (
                investment.date == evaluation_date
            ...
            assert (
                abs(investment.price - closing_price) < 0.01
            ...
            assert (
                abs(investment.amount - monthly_dca_amount) < 0.01
            ...
            assert (
                abs(investment.shares - expected_shares) < 0.0001
    ...
    def test_multiple_investment_execution_accuracy(
        self, investments_data: List[tuple], ticker: str
    ):
        """
        Feature: buy-the-dip-strategy, Property 6: Investment Execution and Recording Accuracy
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            all_investments = investment_tracker.get_all_investments()
            assert len(all_investments) == len(
                executed_investments
            ...
            for i, (eval_date, closing_price, amount) in enumerate(filtered_investments):
                ...
                matching_investments = [
                    ...
                    if (
                        inv.date == eval_date
                        and abs(inv.price - closing_price) < 0.01
                        and abs(inv.amount - amount) < 0.01
                    )
                ]
                assert (
                    len(matching_investments) == 1
                ), f"Should find exactly one matching recorded investment for {eval_date}"

    @given(
        evaluation_date=st.dates(min_value=date(2023, 1, 1), max_value=date(2023, 12, 31)),
        closing_price=st.floats(
            min_value=1.0, max_value=10.0, exclude_min=True
        ),  # Low price for high share count
        monthly_dca_amount=st.floats(min_value=1000.0, max_value=2000.0, exclude_min=True),
    )
    def test_shares_calculation_precision(
        self, evaluation_date: date, closing_price: float, monthly_dca_amount: float
    ):
        """
        Feature: buy-the-dip-strategy, Property 6: Investment Execution and Recording Accuracy
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert (
                abs(investment.shares - expected_shares) < 1e-10
            ...
            assert (
                abs(calculated_amount - investment.amount) < 0.01
    ...
    def test_investment_uses_current_day_closing_price(
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert (
                abs(investment.price - closing_price) < 0.001
    ...
    def test_investment_uses_exact_specified_amount(
        ...
        assume(
            ...
            and config_amount != float("inf")
            and config_amount != float("-inf")
        )
        assume(
            execution_amount == execution_amount
            and execution_amount != float("inf")
            and execution_amount != float("-inf")
        )
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert (
                abs(investment.amount - execution_amount) < 0.01
            ), f"Investment should use exact specified amount: {investment.amount} != {execution_amount}"

            # Verify it doesn't use the config amount
            if (
                abs(config_amount - execution_amount) > 1.0
            ):  # Only check if amounts are significantly different
                assert (
                    abs(investment.amount - config_amount) > 0.1
    ...
    def test_investment_recording_completeness(
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert (
                investment.ticker is not None and investment.ticker != ""
            ), "Investment ticker should be recorded"
            ...
            assert (
                abs(investment.price - closing_price) < 0.01
            ), "Price should be recorded accurately"
            assert (
                abs(investment.amount - monthly_dca_amount) < 0.01
            ), "Amount should be recorded accurately"

            expected_shares = monthly_dca_amount / closing_price
            assert (
                abs(investment.shares - expected_shares) < 0.0001
            ), "Shares should be calculated and recorded accurately"
    ...
    @given(
        evaluation_date=st.dates(min_value=date(2023, 1, 1), max_value=date(2023, 12, 31)),
        closing_price=st.floats(
            min_value=1000.0, max_value=5000.0, exclude_min=True
        ),  # Very high price
        monthly_dca_amount=st.floats(
            min_value=500.0, max_value=1500.0, exclude_min=True
        ),  # Lower amount
    )
