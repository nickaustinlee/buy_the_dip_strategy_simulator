...
class TestStateManager:
    ...
    def config(self):
        ...
        return StrategyConfig(
            ticker="SPY", rolling_window_days=90, percentage_trigger=0.90, monthly_dca_amount=2000.0
        )
    ...
    def test_state_manager_with_complex_state(self, temp_state_dir, config):
        ...
        assert (
            len([tx for tx in loaded_state.all_transactions if tx.session_id == "session-0"]) == 2
        )
        assert (
            len([tx for tx in loaded_state.all_transactions if tx.session_id == "session-1"]) == 2
        )
        assert (
            len([tx for tx in loaded_state.all_transactions if tx.session_id == "session-2"]) == 2
        )
