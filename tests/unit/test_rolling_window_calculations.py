...
class TestRollingWindowCalculations:
    ...
    def create_mock_price_data(self, start_date: date, end_date: date, exclude_weekends: bool = True) -> pd.Series:
        """
    ...
    def test_calendar_days_vs_trading_days_difference(self):
        ...
        trigger_calendar = strategy_calendar.calculate_trigger_price(
            mock_prices, 30, 0.95
        )
        trigger_trading = strategy_trading.calculate_trigger_price(
            mock_prices, 30, 0.95
        )
        
        # They should be different because they use different windows
        assert trigger_calendar != trigger_trading, "Calendar and trading days should produce different results"
        
        # Calendar days should use a shorter actual window (fewer records)
        # so it should have a lower maximum and lower trigger price
        assert trigger_calendar < trigger_trading, "Calendar days should typically have lower trigger (shorter window)"
    ...
    def test_calendar_days_window_boundary(self):
        ...
        assert abs(trigger_price - expected_trigger) < 0.01, f"Expected {expected_trigger}, got {trigger_price}"
    ...
    def test_real_world_scenario_meta_bug(self):
        ...
        assert abs(trigger_calendar - expected_calendar_trigger) < 10.0, \
            f"Calendar trigger should be ~$639, got ${trigger_calendar:.2f}"
        
        assert abs(trigger_trading - expected_trading_trigger) < 10.0, \
            f"Trading trigger should be ~$713, got ${trigger_trading:.2f}"
        
        # Verify they're significantly different
        assert abs(trigger_trading - trigger_calendar) > 30, \
    ...
    def test_edge_case_insufficient_data(self):
        ...
        assert abs(trigger_price - expected_trigger) < 0.01, \
