...
class TestBacktestScenarios:
    ...
    def test_backtest_volatile_market_scenario(self, test_config, volatile_market_data, temp_dir):
        ...
        with patch.object(price_monitor, "get_closing_prices") as mock_get_prices:
            ...
            assert isinstance(result, BacktestResult)
            assert (
                result.total_evaluations > 50
            )  # Should have many evaluation days (relaxed from 60)
            ...
            for i in range(1, len(investments_by_date)):
                ...
                assert (
                    days_between >= 28
                ), f"Investments too close: {days_between} days between {investments_by_date[i-1].date} and {investments_by_date[i].date}"
    ...
    def test_backtest_single_day(self, test_config, temp_dir):
        ...
        historical_data = pd.DataFrame({"Date": all_dates, "Close": all_prices})

        # Mock price data fetch
        with patch.object(price_monitor, "get_closing_prices") as mock_get_prices, patch.object(
            price_monitor, "get_current_price"
        ) as mock_current_price:
    ...
    def test_backtest_preserves_original_investments(
        self, test_config, volatile_market_data, temp_dir
    ):
    ...
    def test_backtest_performance_metrics_calculation(
        self, test_config, bear_market_data, temp_dir
    ):
        ...
        with patch.object(price_monitor, "get_closing_prices") as mock_get_prices, patch.object(
            ...
            if result.investments_executed > 0:
                ...
                expected_percentage_return = (
                    expected_return / expected_invested if expected_invested > 0 else 0.0
                )
