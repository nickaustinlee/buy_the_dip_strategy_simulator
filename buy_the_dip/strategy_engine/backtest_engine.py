...
class BacktestEngine:
    ...
    def run_backtest(
        ...
        rolling_max_series = price_data['Close'].rolling(
        ...
        monthly_investments: Dict[str, float] = {}  # Track total invested per month (YYYY-MM format)
        ...
        for current_date, row in price_data.iterrows():
            try:
                ...
                if not isinstance(current_date, date):
                    logger.error(f"Unexpected date type: {type(current_date)}, value: {current_date}")
                    continue
            ...
            if current_price <= trigger_price:
                ...
                if should_start_new_session:
                    ...
                    logger.info(f"Started DCA session {session_id} on {current_date} "
            ...
            for session in active_sessions:
                ...
                if should_invest:
                    ...
                    if remaining_budget < config.monthly_dca_amount * 0.01:  # Less than 1% of budget remaining
                        ...
                        logger.info(f"Skipped investment on {current_date}: monthly budget exhausted "
                    ...
                    if current_price <= current_trigger_price:
                        ...
                        if transaction:
                            ...
                            days_from_target = (current_date - (last_investment + relativedelta(months=1))).days if last_investment else 0
                            logger.info(f"Investment: ${transaction.amount:.2f} at ${current_price:.2f} "
                        ...
                        last_investment_dates[session.session_id] = current_date
                        logger.info(f"Skipped investment on {current_date}: price ${current_price:.2f} "
