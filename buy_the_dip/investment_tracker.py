...
class InvestmentTracker:
    ...
    def __init__(self, data_dir: Optional[str] = None):
        ...
        """
        if data_dir is None:
            self._data_dir = Path.home() / ".buy_the_dip" / "data"
        else:
    ...
    def add_investment(self, investment: Investment) -> None:
        ...
        if abs(investment.shares - expected_shares) > 0.0001:  # Allow small floating point errors
            logger.warning(
                f"Investment shares calculation mismatch: expected {expected_shares:.6f}, got {investment.shares:.6f}"
            )

        self._investments.append(investment)
        logger.info(
            f"Added investment: {investment.date} - ${investment.amount:.2f} at ${investment.price:.2f}"
        )
    ...
    def has_recent_investment(self, check_date: date, days: int = 28) -> bool:
        ...
        for investment in self._investments:
            if investment.date > cutoff_date and investment.date < check_date:
                logger.debug(
                    f"Found recent investment on {investment.date} within {days} days of {check_date}"
                )
                return True
    ...
    def save_to_file(self, filepath: Optional[str] = None) -> bool:
        ...
        filepath_obj: Path
        if filepath is None:
            filepath_obj = self._data_dir / self.DEFAULT_FILENAME
        else:
            filepath_obj = Path(filepath)

        backup_filepath = filepath_obj.with_suffix(filepath_obj.suffix + self.BACKUP_SUFFIX)

        try:
            # Create backup of existing file if it exists
            if filepath_obj.exists():
                shutil.copy2(filepath_obj, backup_filepath)
            ...
            with open(temp_filepath, "w", encoding="utf-8") as f:
                json.dump(
                    {"investments": investments_data, "last_updated": datetime.now().isoformat()},
                    f,
                    indent=2,
                    default=str,
                )
            ...
            logger.info(
                f"Successfully saved {len(self._investments)} investments to {filepath_obj}"
            )
            return True

        except Exception as e:
            logger.error(f"Failed to save investments to {filepath_obj}: {e}")
            return False
    ...
    def load_from_file(self, filepath: Optional[str] = None) -> bool:
        ...
        success = self._load_from_specific_file(filepath_obj)

        if not success and backup_filepath.exists():
            logger.warning(
                "Main investment file corrupted or missing, attempting to load from backup"
            )
            ...
            if success:
                ...
                self.save_to_file(str(filepath_obj))
    ...
    def _handle_corrupted_file(self, filepath: Path) -> None:
        ...
        """
        try:
            corrupted_backup = filepath.with_suffix(
    ...
    def get_investments_in_period(self, start_date: date, end_date: date) -> List[Investment]:
        ...
        """
        return [inv for inv in self._investments if start_date <= inv.date <= end_date]
