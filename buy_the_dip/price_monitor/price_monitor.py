...
class PriceMonitor:
    ...
    def _get_market_calendar(self) -> Any:
    ...
    def _log_no_data_reason(self, ticker: str, start_date: date, end_date: date) -> None:
        ...
        if all_non_trading:
            if start_date == end_date:
                ...
                logger.debug(
                    f"No data for {ticker} on {start_date} ({day_name}) - not a trading day"
                )
            else:
                logger.debug(
                    f"No data for {ticker} from {start_date} to {end_date} - range contains only non-trading days"
                )
        else:
            # Some trading days in range, but still no data - this might be more concerning
            logger.warning(
                f"No price data available for {ticker} from {start_date} to {end_date} - "
    ...
    def _load_cached_data(self, ticker: str) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            # Convert Date column to date objects - use explicit copy to avoid warnings
            df_copy = df.copy()
            df_copy["Date"] = pd.to_datetime(df_copy["Date"]).dt.date  # type: ignore[index]
            df = df_copy
    ...
    def _save_cached_data(self, ticker: str, data: pd.DataFrame) -> None:
        ...
        try:
            ...
            for record in cache_data["prices"]:  # type: ignore[index]
                if isinstance(record["Date"], date):  # type: ignore[index]
                    record["Date"] = record["Date"].isoformat()  # type: ignore[index]
    ...
    def _merge_cached_and_new_data(
    ...
    def fetch_price_data(
        self, ticker: str, start_date: date, end_date: date, ignore_cache: bool = False
    ) -> pd.DataFrame:
        """
        ...
        for range_start, range_end in missing_ranges:
            ...
            try:
                ...
                if not data.empty:
                    ...
                    all_new_data = pd.concat([all_new_data, range_data_copy], ignore_index=True)
                else:
                ...
                if self._is_likely_non_trading_day(range_start) and self._is_likely_non_trading_day(
                    range_end
                ):
                    logger.debug(
                        f"No data for {ticker} ({range_start} to {range_end}) - likely non-trading days"
                    )
                else:
                    logger.error(
                        f"Failed to fetch price data for {ticker} ({range_start} to {range_end}): {e}"
                    )
    ...
    def _fetch_fresh_data(self, ticker: str, start_date: date, end_date: date) -> pd.DataFrame:
        ...
        try:
            ...
            if not data.empty:
                ...
                return fresh_data_copy  # type: ignore[return-value, no-any-return]
            else:
                self._log_no_data_reason(ticker, start_date, end_date)
                return pd.DataFrame()
            ...
            logger.error(
                f"Failed to fetch fresh price data for {ticker} ({start_date} to {end_date}): {e}"
            )
            return pd.DataFrame()
    ...
    def is_cache_valid(self, ticker: str, cache_days: int = 30) -> bool:
        ...
        if hasattr(latest_cached_date, "date"):
            latest_cached_date = latest_cached_date.date()
        elif isinstance(latest_cached_date, str):
            # Handle string dates
            from datetime import datetime

            latest_cached_date = datetime.strptime(latest_cached_date, "%Y-%m-%d").date()

        # Ensure we have a date object
        if not isinstance(latest_cached_date, date):
            return False
    ...
    def get_current_price(self, ticker: str) -> float:
        ...
        if cached_data is not None and not cached_data.empty:
            ...
            if (date.today() - latest_cached_date).days <= 1:
                latest_price = cached_data[cached_data["Date"] == latest_cached_date]["Close"].iloc[
                    -1
                ]
        ...
        try:
            ...
            if data.empty:
                ...
                if self._is_likely_non_trading_day(today):
                    ...
                    if today.weekday() >= 5:
                        raise ValueError(
                            f"No current price data for {ticker} - markets are closed on {day_name}s"
                        )
                    else:
                        raise ValueError(
                            f"No current price data for {ticker} - markets appear to be closed today ({day_name}, likely a holiday)"
                        )
                else:
                    raise ValueError(
                        f"No current price data available for {ticker} - check if ticker is valid"
                    )
    ...
    def get_cache_info(self, ticker: str) -> Dict:
        ...
        if cached_data is None or cached_data.empty:
            return {"ticker": ticker, "cached": False, "records": 0, "date_range": None}
