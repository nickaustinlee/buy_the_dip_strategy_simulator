...
class StrategyConfig(BaseModel):
    ...
    rolling_window_days: int = Field(
        default=90, ge=1, le=365, description="Number of days for rolling maximum calculation"
    )
    percentage_trigger: float = Field(
        default=0.90, gt=0.0, le=1.0, description="Percentage of rolling maximum that triggers DCA"
    )
    monthly_dca_amount: float = Field(
        default=2000.0, gt=0.0, description="Monthly dollar amount for DCA investments"
    )
    data_cache_days: int = Field(default=30, ge=1, description="Number of days to cache price data")
