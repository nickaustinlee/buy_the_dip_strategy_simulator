...
from ..strategy_system import StrategySystem, BacktestResult, EvaluationResult
...
def validate_cached_data(
    price_monitor: PriceMonitor, ticker: str, max_records: int = 50
) -> Dict[str, Any]:
    """
        ...
        if fresh_data.empty:
            return {
                ...
                "sampling_info": {
                    ...
                    "validation_strategy": "full_cache"
                    if date_range_days < 30 or total_records <= max_records
        ...
        return {
            ...
            "sampling_info": {
                ...
                "random_percentage": round(
                    (random_count / max(1, total_records - recent_count)) * 100, 1
                )
                if random_count > 0
                else 0,
                "validation_strategy": "full_cache"
...
def format_backtest_result(result: BacktestResult, config: Any, price_monitor: PriceMonitor) -> str:
    ...
    if result.all_investments:
        try:
            ...
            logging.getLogger(__name__).debug(
                f"After buy-and-hold fetch - API calls: {api_stats_after_bh['api_calls_made']}, Cache hits: {api_stats_after_bh['cache_hits']}"
            )
    ...
    if result.all_investments:
        ...
        for inv in result.all_investments:
            lines.append(
                f"{inv.date}: ${inv.amount:,.2f} at ${inv.price:.2f} = {inv.shares:.4f} shares"
            )
    else:
...
def format_evaluation_result(result: EvaluationResult, config: Any) -> str:
    ...
    lines.append(
        f"Recent Investment Exists: {'✅ YES' if result.recent_investment_exists else '❌ NO'}"
    )
    lines.append("")

    if result.investment_executed:
        lines.append("🚀 INVESTMENT EXECUTED!")
        if result.investment is not None:
...
def format_multi_ticker_check(
    results: List[Dict[str, Any]], check_date: date, use_latest_price: bool = False
) -> str:
    ...
    lines.append(
        f"{'Ticker':<8} {'Closing':<12} {'Trigger':<12} {'Signal':<8} {'% from Trigger':<15}"
    )
    lines.append("-" * 80)
    ...
    for result in results:
        ...
        lines.append(
            f"{ticker:<8} ${yesterday_price:<11.2f} ${trigger_price:<11.2f} {signal:<8} {pct_str:<15}"
        )
...
def create_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--backtest", action="store_true", help="Run backtest for historical evaluation"
    )
    ...
    parser.add_argument(
        "--status", action="store_true", help="Show current portfolio status and metrics"
    )
    ...
    parser.add_argument(
        "--start-date", type=str, help="Backtest start date (YYYY-MM-DD format, e.g., 2023-01-01)"
    )

    parser.add_argument(
        "--end-date", type=str, help="Backtest end date (YYYY-MM-DD format, e.g., 2024-12-31)"
    )

    parser.add_argument(
        "--period", type=str, help="Backtest period from end date (e.g., '1y', '6m', '90d', '2y')"
    )
    ...
    parser.add_argument(
        "--cache-info", type=str, help="Show cache information for a specific ticker"
    )
    ...
    parser.add_argument(
        "--validate-cache", type=str, help="Validate cached data against live API data for a ticker"
    )
...
def main() -> None:
    ...
    try:
        ...
        if args.tickers:
            ...
            for ticker in args.tickers:
                ...
                try:
                    ...
                    trigger_price = strategy_system.calculate_trigger_price(
                        historical_prices, config.rolling_window_days, config.percentage_trigger
                    )
                    ...
                    if config.use_trading_days:
                        ...
                        if len(historical_prices) > 0:
                            ...
                            window_prices = historical_prices[
                                historical_prices.index >= cutoff_date
                            ]
                        else:
                            window_prices = historical_prices
            ...
            logger.info(
                f"Session total - API calls: {api_stats['api_calls_made']}, Cache hits: {api_stats['cache_hits']}"
            )
        ...
        if args.cache_info:
            ...
            if cache_info["cached"]:
                ...
                print(
                    f"Date Range: {cache_info['date_range']['start']} to {cache_info['date_range']['end']}"
                )
            else:
            ...
            try:
                ...
                if "sampling_info" in validation_result:
                    ...
                    if info["validation_strategy"] == "full_cache":
                        ...
                        print(
                            f"Recent Records Checked: {info['recent_records_checked']} ({info['recent_percentage']}% of total)"
                        )
                        if info["random_records_checked"] > 0:
                            print(
                                f"Random Sample Checked: {info['random_records_checked']} ({info['random_percentage']}% of remaining)"
                            )
                ...
                if validation_result["mismatches"] > 0:
                    ...
                    if validation_result.get("sample_mismatches"):
                        ...
                        for mismatch in validation_result["sample_mismatches"][:3]:
                            print(
                                f"  {mismatch['date']}: Cached=${mismatch['cached']:.2f}, API=${mismatch['api']:.2f}"
                            )
                else:
        ...
        if args.backtest:
            # Run backtest
            try:
                start_date_opt, end_date_opt = resolve_date_range(args)

                # Ensure dates are not None for backtest
                if start_date_opt is None or end_date_opt is None:
                    logger.error("Invalid date range for backtest")
                    sys.exit(1)

                # Now we know they're not None
                backtest_start_date: date = start_date_opt
                backtest_end_date: date = end_date_opt

                logger.info(f"Running backtest from {backtest_start_date} to {backtest_end_date}")
                backtest_result = strategy_system.run_backtest(
                ...
                formatted_result = format_backtest_result(backtest_result, config, price_monitor)
                print(formatted_result)
            ...
            try:
                ...
                evaluation_result = strategy_system.evaluate_trading_day(eval_date)

                formatted_result = format_evaluation_result(evaluation_result, config)
                print(formatted_result)

                # Send notification if investment was executed
                if (
                    args.notify
                    and evaluation_result.investment_executed
                    and evaluation_result.investment is not None
                ):
                    from datetime import datetime

                    current_time = datetime.now()
                    timestamp_str = current_time.strftime("%B %d, %Y @ %I:%M %p Local")
                    message = f"Investment executed! ${evaluation_result.investment.amount:.0f} in {config.ticker} at ${evaluation_result.investment.price:.2f}\\n\\nTS: {timestamp_str}"
