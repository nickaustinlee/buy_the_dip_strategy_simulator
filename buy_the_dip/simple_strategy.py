...
class SimpleStrategy:
    ...
    def __init__(self, price_monitor: PriceMonitor = None):
        self.price_monitor = price_monitor or PriceMonitor()
    
    def run_backtest(self, config: StrategyConfig, start_date: date, end_date: date) -> StrategyResult:
        """
        ...
        rolling_max = price_data['Close'].rolling(
        ...
        while current_date <= end_date:
            try:
                ...
                current_price = float(price_data.loc[trading_date, 'Close'])
                current_rolling_max = float(rolling_max.loc[trading_date])
                ...
                if current_price <= trigger_price:
                    ...
                    logger.info(f"INVEST: {trading_date} - ${config.monthly_dca_amount:,.0f} at ${current_price:.2f} "
                               f"(trigger: ${trigger_price:.2f}, rolling max: ${current_rolling_max:.2f})")
                else:
                    # Skip
                    logger.info(f"SKIP: {trading_date} - price ${current_price:.2f} > trigger ${trigger_price:.2f} "
    ...
    def _find_closest_trading_day(self, price_data: pd.DataFrame, target_date: date) -> date:
    ...
    def format_results(self, result: StrategyResult, ticker: str, start_date: date, end_date: date) -> str:
        ...
        if result.investments:
            ...
            for i, inv in enumerate(result.investments, 1):
                lines.append(f"  {i}. {inv.date} - ${inv.amount:,.0f} at ${inv.price:.2f} = {inv.shares:.4f} shares")
        else:
